       ...
       <Tabs defaultValue="members" className="w-full">
          <div className='flex flex-row items-center  justify-between'>
            <TabsList className="w-[400px] grid  gap-x-2 grid-cols-2 bg-none rounded-md ">
                <TabsTrigger onClick={()=> setActiveTab('members')} value="members" className={cn("todo_wrapper_tabs",activeTab=="members"?"active_tab":"")}> 
                  <div><User size={16}/>Membres</div>
                   <span className='font-bold rounded-full  dark:bg-dark-secondary bg-light-secondary'>{allUsers.length}</span>
                </TabsTrigger>
            ...
            <TabsContent value="members" className='h-full w-full'>
              <section className='h-full'>
                <ScrollArea className='h-full'>
                  <UsersTable allUsers={allUsers} openAddUserDialog={openAddUserDialog} setOpenAddUserDialog={setOpenAddUserDialog} refreshLayout={refreshLayout}/>
                </ScrollArea>
              </section>
            </TabsContent>
            <TabsContent value="groups"  className='h-full w-full'>
              <section className='h-full'>
                  <ScrollArea className='h-full'>
                    <GroupsTable allUsers={allUsers} openCreateGroup={openCreateGroup} setOpenCreateGroup={setOpenCreateGroup} refreshLayout={refreshLayout}/>
                  </ScrollArea>
                </section>
            </TabsContent>
        </Tabs>

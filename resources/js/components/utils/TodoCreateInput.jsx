                        ...
                        {isAssign ?
                           assignedUsers.length != 0?
                            ...
                            .map((user,index) =>(
                             ...
                             { dueDate !=null ? 
                                <div>
                                    {dueDate !=null ?<span className='text-[12px]'><i className='bx bx-calendar-event text-[#fff] text-[12px] mr-1'></i>{format(dueDate, 'dd MMM yyyy')}</span>:""}
                                </div>
                                :""}
                    </div>
                        ...
                        <PopoverTrigger>
                            <TooltipProvider >
                                ...
                                <TooltipTrigger asChild className='cursor-pointer'>
                                    <span><i className='bx bx-calendar-event text-[#fff] text-[20px]'></i></span>
                                </TooltipTrigger>
                                <TooltipContent className='bg-[#313131] border-none text-[#fff] z-10'>
                            ...
                            <PopoverContent side='top' className={cn('bg-[#d8d8d833] border-none flex gap-x-3 text-[13px] text-[#fff] flex-col dmax-h-[150px] w-[180px] p-2',showCalendar?"w-full":"w-[180px]")}>
                               ...
                               <div className={cn('flex flex-col', showCalendar?"hidden":"block")}>
                                    <div onClick={()=> setDueDate(format(date, 'yyyy-MM-dd'))} className='relative cursor-pointer flex flex-row items-center justify-between gap-x-2 hover:bg-[#d8d8d833] p-1 rounded-[4px]'>
                                            <div className='flex flex-row items-center gap-x-2'>
                                                <i className='bx bx-calendar-event text-[#fff] text-[14px]'></i>Aujourd'hui
                                            </div>
                                    ...
                                    <div onClick={()=> setDueDate(format(addDays(date, 1), 'yyyy-MM-dd'))}  className='relative cursor-pointer flex flex-row items-center justify-between gap-x-2  hover:bg-[#d8d8d833] p-1 rounded-[4px]'>
                                            <div className='flex flex-row items-center gap-x-2'>
                                                <i class='bx bx-calendar-plus text-[#fff] text-[14px]'></i>Demain
                                            </div>
                                    ...
                                    <div onClick={()=> setShowCalendar(true)}  className='relative cursor-pointer flex flex-row items-center justify-between gap-x-2 hover:bg-[#d8d8d833] p-1 rounded-[4px]'>
                                            <div className='flex flex-row items-center gap-x-2'>
                                                <i class='bx bx-calendar text-[#fff] text-[14px]'></i>Choisir une date
                                            </div>
                                            <span><i class='bx bx-chevron-right text-[#fff] text-[18px]'></i></span>
                                    </div>
                               </div>
                        ...
                        <Popover>
                            <PopoverTrigger>
                                <TooltipProvider >
                                    ...
                                    <TooltipTrigger asChild className='cursor-pointer'>
                                    <div>
                                        <span><i class='bx bxs-user text-[#fff] text-[20px]'></i></span>
                                    </div>
                                    </TooltipTrigger>
                                    <TooltipContent className='bg-[#313131] border-none text-[#fff] z-10'>
                            ...
                            <PopoverContent className='bg-[#d8d8d833] border-none flex flex-col max-h-[150px] w-[80%] p-2'>
                                {users.map((user,index) =>(
                                        <div key={index} className={cn('relative hover:cursor-pointer p-2 rounded-[4px]', assignedUsers.length !=0?(assignedUsers.includes(user.id)?"opacity-[0.5]":"opacity-[1]"):" hover:bg-[#d8d8d833]")} onClick={()=> addUsersToTask(user.id)}>
                                        <div className='flex flex-row items-center gap-x-1'>
                                            <div className='w-[20px] h-[20px]'>
                                                <img src={user.avatar} alt="avatar" className=" object-fit-contain rounded-full" />
                                            </div>
                                            <h1 className='text-white text-[12px] upload_file_name'>{user.name}</h1>
                                        </div>
                                        {assignedUsers.includes(user.id)?
                                        <span className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'><i class='bx bx-check bx-md text-[#fff] opacity-1'></i></span>
                                        :""}
                                        </div>
                                    ))}
                            </PopoverContent>

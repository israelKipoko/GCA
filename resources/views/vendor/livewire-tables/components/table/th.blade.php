...
@if ($component->isTailwind())
    <th scope="col" {{
        $attributes->merge($customAttributes)
            ->class(['px-6 py-3 text-center text-[14px] font-bold whitespace-nowrap text-white capitalize  tracking-wider dark:bg-[#262626] dark:text-gray-400' => $customAttributes['default'] ?? true])
                ...
                <button
                    ...
                    {{
                        $attributes->merge($customSortButtonAttributes)
                            ->class(['flex items-center space-x-1  text-center text-md leading-4 font-bold text-white tracking-wider group focus:outline-none dark:text-gray-400' => $customSortButtonAttributes['default'] ?? true])
                            ->except(['default', 'wire:key'])
                    }}
                >
                    <span class="text-white text-[15px] font-bold text-center capitalize">{{ $column->getTitle() }}</span>
